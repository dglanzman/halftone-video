...
int main() {
    ...
    // setup the window
    const char * window_name = "Halftone";
    int dot_size = 12;
    namedWindow(window_name, CV_WINDOW_AUTOSIZE);
    createTrackbar("Dot Size:", window_name, &dot_size, 128, NULL);

    for (;;) {
        Mat frame;
        camera >> frame;
        if (frame.empty()) continue;
        Mat result = make_halftone(frame, dot_size);
